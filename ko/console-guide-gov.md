...
2. **스크립트 배포**를 클릭합니다.
3. 웹 터미널, 인증정보 입력방식을 선택 후, 인증정보를 입력합니다.
   * 전체 : 배포대상 인스턴스들의 인증정보가 모두 동일할 경우 사용합니다.
   * 개별 : 배포대상 인스턴스들의 인증정보가 다를 경우 사용합니다.
4. **배포**를 클릭합니다.
5. 스크립트 배포현황에서 실행결과 및 배포결과를 확인할 수 있습니다.
6. **배포 종료**를 통해 배포를 중단할 수 있습니다.
7. **배포 현황**을 클릭하여 진행중인 배포 현황 및 결과를 볼 수 있습니다.
...
> [참고]
> 관리자가 직접 계정을 잠금하는 경우에는 **환경 설정**의 계정 잠금 시간과 상관없이 무기한 잠금됩니다.
...
사용자가 금지된 명령어를 사용하여 일정 횟수 이상 명령어가 차단될 경우 세션을 차단할 수 있습니다.

#### 계정 잠금

인증 실패 및 세션 차단됨에 따라 계정을 잠금할 수 있습니다.
* 계정 잠금 시간 : 계정 잠금될 경우 잠금 시간 설정합니다.
* 연속적인 세션 차단 : 연속적으로 일정 횟수 이상 세션 차단되는 경우 계정이 잠금됩니다.
* 시간당 세션 차단 : 한 시간내에 일정 횟수 이상 세션이 차단될 경우 계정이 잠금됩니다.
* 연속적인 인증 실패 : 연속적으로 일정 횟수 이상 인증에 실패하는 경우 계정이 잠금됩니다.
* 시간당 인증 실패 : 한 시간내에 일정 횟수 이상 인증에 실패하는 경우 계정이 잠금됩니다.

> [참고]
> **사용자 관리**에서 관리자가 직접 계정을 잠금하는 경우에는, 계정 잠금 시간과 상관없이 무기한 잠금됩니다.
